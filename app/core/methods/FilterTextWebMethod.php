...
class FilterTextWebMethod extends WebMethod
{
	...
	function setStyle( $style )
	{
 	...
 	function drawSelect( $parms_array = array() ) 
 	{
		...
		echo '<input type="text" valueparm="'.$this->getValueParm().'" class="btn-sm input-large" placeholder="'.$this->title.'" style="'.$this->getStyle().'" value="'.htmlentities($value_text).'" onchange="'.$this->script.'" onkeyup="enterKeyUp(event) ? '.preg_replace('/javascript:/','',$this->script).' : 0;">';
 	}
