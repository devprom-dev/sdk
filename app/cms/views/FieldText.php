...
class FieldText extends Field
{
 	...
 	function draw( $view = null )
	{
		...
		else
		{
			...
			if ( $this->rows == 1 )
			{
				echo '<input id="'.$this->getId().'" name="'.$this->getName().'" class="input-block-level" autocomplete="off" type="text" tabindex="'.$this->getTabIndex().'" placeholder="'.htmlentities($this->getDefault()).'" value="'.htmlentities($this->getValue()).'" '.($this->getRequired() ? 'required' : '').' autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">';
			}
			else
			{
