...
class PlanChartDataRegistry extends ObjectRegistrySQL
{
 	...
 	{
        $items = array(
            " SELECT IFNULL(t.MilestoneDate, NOW()) StartDate, DATE(t.MilestoneDate) FinishDate, ".
            ...
            "		 'Milestone' ObjectType, 0 SortIndex, '' Custom1, 0 UncompletedTasks, 0 UncompletedIssues, 0 `Release` ".
            "   FROM pm_Milestone t ".
            "  WHERE 1 = 1 ".getFactory()->getObject('Milestone')->getVpdPredicate('t'),

			" SELECT IFNULL(t.StartDate, NOW()) StartDate, IFNULL(DATE(t.FinishDate),DATE_ADD(NOW(),INTERVAL 3 YEAR)) FinishDate, t.Project, t.Caption, ".
            ...
            "						   AND m.Metric = 'EstimatedFinish'), t.FinishDate) EstimatedFinishDate, ".
            ...
            "        0 UncompletedTasks, t.pm_VersionId ".
            "   FROM pm_Version t ".
            ...
            "			AND s.FinishDate IS NULL) UncompletedTasks, t.Version ".
 			"   FROM pm_Release t ".
