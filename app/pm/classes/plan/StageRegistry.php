...
class StageRegistry extends ObjectRegistrySQL
{
 	function getQueryClause()
 	{
		...
		$projectUsed = getSession()->getProjectIt()->get('VPD');

 		$sql = " SELECT LPAD(v.Caption, 8, '0') VersionNumber, " .
 			   "		v.Caption Caption, ".
               "        IF(v.VPD != '".$projectUsed."', CONCAT('{',(SELECT p.Caption FROM pm_Project p WHERE p.VPD = v.VPD),'} '), '') CaptionPrefix, ".
               ...
               "        (SELECT GROUP_CONCAT(CAST(s.pm_ChangeRequestId AS CHAR)) FROM pm_ChangeRequest s " .
               "	      WHERE s.PlannedRelease = v.pm_VersionId AND s.Type IS NOT NULL AND s.Iteration IS NULL ) Increments, ".
		...
		$sql .= 			   
           ...
           "        IF(r.VPD != '".$projectUsed."', CONCAT('{',(SELECT p.Caption FROM pm_Project p WHERE p.VPD = r.VPD),'} '), ''), ".
           ...
           "       (SELECT GROUP_CONCAT(CAST(a.pm_ChangeRequestId AS CHAR)) FROM pm_ChangeRequest a 
                          WHERE a.Iteration = r.pm_ReleaseId AND a.Type IS NOT NULL) Increments, ".
		   "         0 UncompletedIssues, ".
    	   ...
    	   "		r.RecordModified, ".
    	   "		UNIX_TIMESTAMP(v.RecordModified) * 100000 AffectedDate, ".
           "        CONCAT(LPAD(v.pm_VersionId, 8, '0'),LPAD(r.pm_ReleaseId, 8, '0')) Stage ".
    	   "   FROM (SELECT r.*, ".
