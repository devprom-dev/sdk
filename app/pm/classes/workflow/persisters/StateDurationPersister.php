	...
	function getSelectColumns( $alias )
 	{
        ...
        $columns[] = "( SELECT UNIX_TIMESTAMP(NOW()) / 3600 - 
        ...
        if ( $this->getObject()->getAttributeType('LifecycleDuration') != '' ) {
            $columns[] =
                "(IFNULL({$alias}.LifecycleDuration, 0) + 
			        ...
			        (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP((SELECT IFNULL(MAX(so.RecordCreated), {$alias}.RecordCreated) 
			                                                   FROM pm_StateObject so WHERE so.pm_StateObjectId = {$alias}.StateObject) )) / 3600, 0) ) LeadTime ";
        }
        else {
            $columns[] =
                "(SELECT IFNULL( 
