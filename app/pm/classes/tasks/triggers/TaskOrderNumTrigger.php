...
class TaskOrderNumTrigger extends SystemTriggersBase
{
	...
	function processOrderNum( $object_it )
	{
		...
		$sql = "UPDATE pm_Task t SET t.OrderNum = @r:= (@r+1), t.RecordModified = NOW() WHERE t.pm_TaskId IN (".join(",", $seq_it->idsToArray()).") ORDER BY t.OrderNum ASC";
		...
		$sql = "INSERT INTO co_AffectedObjects (RecordCreated, RecordModified, VPD, ObjectId, ObjectClass) ".
		       ...
		       "     FROM pm_Task t WHERE t.pm_TaskId IN (".join(",", $seq_it->idsToArray()).") ";
