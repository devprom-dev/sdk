    ...
    private function getBoundary()
    {
        ...
        while(!feof($this->stream)) {
            ...
            if(($pos = strpos($line, self::STR_BOUNDARY_PREFIX)) !== FALSE) {
                preg_match('/'.self::STR_BOUNDARY_PREFIX.'([^"]+)"/i', $line, $matches);
    ...
    public function parse()
    {
        ...
        foreach($this->parts as $i => $part) {
            ...
            if($part['type'] == 'image' && !isset($this->imageNameMap[$part['image_file']])) {
                ...
                fseek($fp, $part['start']);
                $oldFilePath = realpath($this->outputDir) .
                    DIRECTORY_SEPARATOR . basename(dirname($part['image_file'])) .
                    DIRECTORY_SEPARATOR . basename($part['image_file']);
                mkdir(dirname($oldFilePath), 0777, true);
        ...
        foreach($this->parts as $i => $part) {
            ...
            if($part['type'] == 'text') {
                ...
                list($part['format'], $charset) = preg_split('/;/', $part['format']);
