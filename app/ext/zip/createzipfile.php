...
class createFileZip  {  
	...
	function createFileZip( $file_path ) {
		$this->file = fopen( $file_path, 'wb' );
	}
	...
	function addDirectory($directoryName) {
		...
		fwrite( $this->file, $feedArrayRow, strlen($feedArrayRow) );
		$newOffset = ftell( $this->file );
	...
	function saveZippedfile() 
	{ 
		$data_length = ftell( $this->file );
		$controlDirectory = implode("", $this -> centralDirectory);
		  
		$data = $controlDirectory.  
			$this -> endOfCentralDirectory.  
			pack("v", sizeof($this -> centralDirectory)).     
			pack("v", sizeof($this -> centralDirectory)).     
			pack("V", strlen($controlDirectory)).             
		...
		fwrite( $this->file, $data, strlen($data) );
		fclose( $this->file );
	}
}
