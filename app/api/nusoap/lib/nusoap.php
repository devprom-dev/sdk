...
class nusoap_base {
	...
	"</SOAP-ENV:Envelope>";
		...
		/**
		 * sets the HTTP content type charset for the current response.
		 * returns false for non-text content types.
		 */
		function setHTTPContentTypeCharset( $defencoding )
		{
			$this->soap_defencoding = $defencoding;
		}
}
...
class nusoap_server extends nusoap_base {
	...
	function serialize_return() {
		...
		// if fault
		if (is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'soap_fault') || (get_class($this->methodreturn) == 'nusoap_fault'))) {
		...
		if (count($this->bindings) >= 1) {
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					$binding_xml .= "\n" . '    <input><soap:body use="' . $opParts['input']['use'] . '" ' . $enc_style . '/></input>';
					if (isset($opParts['output']['encodingStyle']) && $opParts['output']['encodingStyle'] != '') {
						$enc_style = ' encodingStyle="' . $opParts['output']['encodingStyle'] . '"';
					} else {
						$enc_style = '';
					}
					$binding_xml .= "\n" . '    <output><soap:body use="' . $opParts['output']['use'] . '" ' . $enc_style . '/></output>';
					$binding_xml .= "\n" . '  </operation>';
