...
public class RequirementsScreenCast extends ProjectTestBase{
    ...
    private void documentationStage() throws InterruptedException {
        ...
        RequirementNewPage newPage = requirementsDocsPage.addDoc();
        RequirementViewPage newDocPage = newPage.createSimple(new Requirement("Требования к комплексу дистанционного обучения"));
        ...
        subSection2.setType("Бизнес-требование");
        reqNewPage.createWithHTML(subSection2,brTemplate.replace("%text%", brText).replace("%model%",brModel));
        newDocPage.addChildRequirement();
        subSection3 = new Requirement("Ограничения");
        subSection3.setContent(" ");
        subSection3.setType("Бизнес-требование");
        reqNewPage.createWithTemplate(subSection3);
        ...
        RequestViewPage viewPage = inboxPage.clickOnWish(wish1.getId());
        viewPage.doAnalyse("");
    ...
    private void agreementStage() throws InterruptedException {
        ...
        addToBaselinePage.Submit(baselineReq, "Релиз 0");
        ...
        requirementViewPage.compareWithVersion("Начальный");
        Thread.sleep(3000);
    }
