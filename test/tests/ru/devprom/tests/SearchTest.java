...
public class SearchTest extends ProjectTestBase {
	...
	public void createRequests() throws InterruptedException {
		...
		SearchResultsPage srp = favspage.searchByKeyword(nameKeyword);
		...
		for (SearchResultItem item:results){
            ...
            Assert.assertEquals(foundRequirement.getBold(),nameKeyword, "Can't find search string in Requirement bold text");
            
            Assert.assertTrue(foundRequest.getFindString().contains("Название:"), "Search area is incorrect - should be 'Название'");
            Assert.assertEquals(foundRequest.getBold(),nameKeyword, "Can't find search string in Request bold text");
            
            Assert.assertTrue(foundTask.getFindString().contains("Название:"), "Search area is incorrect - should be 'Название'");
            Assert.assertEquals(foundTask.getBold(),nameKeyword, "Can't find search string in Request bold text");
            
            //Search in content
            srp = srp.searchByKeyword(contentKeyword);
    		...
    		for (SearchResultItem item:results){
                ...
                Assert.assertEquals(foundRequest.getBold(),contentKeyword, "Can't find search string in Requirement bold text");
                Assert.assertEquals(foundRequirement.getBold(),contentKeyword, "Can't find search string in Request bold text");                
