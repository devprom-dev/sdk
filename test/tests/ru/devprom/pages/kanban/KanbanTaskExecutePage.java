...
public class KanbanTaskExecutePage extends KanbanPageBase {
	...
	public KanbanTaskViewPage subtaskExecute(double estimates){
		driver.findElement(By.xpath("//input[@value='activitytask']/following-sibling::a[contains(@class,'embedded-add-button')]")).click();
		driver.findElement(By.xpath("//span[@id='pm_TaskFact']//input[contains(@id,'Capacity')]")).sendKeys(String.valueOf(estimates));
		driver.findElement(By.xpath("//span[@id='pm_TaskFact']//input[contains(@id,'saveEmbedded')]")).click();
		submitDialog(submitBtn);
		return new KanbanTaskViewPage(driver);
	}
}
